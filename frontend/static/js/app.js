...
async function editParameter(parameterName) {
    ...
    const logData = {
        ...
        hypothesisId: 'H',
        location: 'app.js:658',
        message: 'editParameter called',
        data: { parameterName },
        timestamp: Date.now()
    };
    fetch('http://127.0.0.1:7242/ingest/a25507bb-3540-4e9f-bc1f-49484404c78f', {
        method: 'POST',
    ...
    try {
        ...
        document.getElementById('car-selector-group').style.display = 'none';
        ...
        const modal = document.getElementById('edit-parameter-modal');
        if (modal) {
            modal.style.display = 'block';
            modal.style.pointerEvents = 'auto'; // Ensure pointer events enabled when visible
        }
...
function closeEditParameter() {
    ...
    if (modal) {
        ...
        modal.style.pointerEvents = 'none'; // Ensure pointer events disabled when hidden
    }
...
async function loadCarsForSelector() {
    try {
        ...
        const carSelect = document.getElementById('edit-car-id');
        if (carSelect) {
            // Clear existing options except the first one
            carSelect.innerHTML = '<option value="">No specific car (apply to all)</option>';
            
            // Add cars
            carsList.forEach(car => {
                const option = document.createElement('option');
                option.value = car.car_identifier;
                option.textContent = car.display_name || car.car_identifier;
...
// Toggle car selector visibility
function toggleCarSelector() {
    const queueCheckbox = document.getElementById('edit-queue');
    const carSelectorGroup = document.getElementById('car-selector-group');
    if (queueCheckbox && carSelectorGroup) {
        carSelectorGroup.style.display = queueCheckbox.checked ? 'block' : 'none';
    }
}

// Filter queue by car
async function filterQueueByCar() {
    const carFilter = document.getElementById('queue-car-filter');
    if (carFilter) {
        queueCarFilter = carFilter.value || null;
        await loadQueue(queueCarFilter);
    }
}

// Load cars for queue filter
async function loadCarsForQueueFilter() {
    try {
        const response = await fetch('/api/cars', {
            credentials: 'include'
        });
        ...
        console.error('Error loading cars for queue filter:', error);
    }
}

window.editParameter = editParameter;
window.closeEditParameter = closeEditParameter;
window.toggleCarSelector = toggleCarSelector;
window.filterQueueByCar = filterQueueByCar;
window.showHistory = showHistory;
window.closeHistory = closeHistory;
window.showFormHistory = showFormHistory;
...
window.permanentlyRemoveDeletedUser = permanentlyRemoveDeletedUser;
...
async function showHistory(parameterName) {
    try {
        ...
        if (historyModal) {
            ...
            historyModal.style.pointerEvents = 'auto'; // Ensure pointer events enabled when visible
        }
    } catch (error) {
...
window.onclick = function(event) {
    ...
    const logData = {
        ...
        hypothesisId: 'E',
        location: 'app.js:859',
        message: 'Window click event',
        data: {
            target: event.target?.tagName || 'unknown',
            targetId: event.target?.id || 'none',
            targetClass: event.target?.className || 'none',
            isModal: event.target?.classList?.contains('modal') || false
        },
        timestamp: Date.now()
    };
    fetch('http://127.0.0.1:7242/ingest/a25507bb-3540-4e9f-bc1f-49484404c78f', {
    ...
    const editModal = document.getElementById('edit-parameter-modal');
...
// Car filter for queue
let queueCarFilter = null;

async function loadQueue(carId = null) {
    try {
        let url = '/api/queue?status=pending';
        if (carId) {
...
async function showUserManagement() {
    try {
        ...
        if (usersModal) {
            ...
            usersModal.style.pointerEvents = 'auto'; // Ensure pointer events enabled when visible
        }
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const logData = {
        ...
        runId: 'run1',
        hypothesisId: 'F',
        location: 'app.js:1266',
        message: 'DOMContentLoaded - checking elements',
        data: {
            editFormExists: !!document.getElementById('edit-parameter-form'),
            editModalExists: !!document.getElementById('edit-parameter-modal'),
            historyModalExists: !!document.getElementById('history-modal'),
            usersModalExists: !!document.getElementById('users-modal'),
            parametersTableExists: !!document.getElementById('parameters-tbody')
        },
        timestamp: Date.now()
    };
    fetch('http://127.0.0.1:7242/ingest/a25507bb-3540-4e9f-bc1f-49484404c78f', {
    ...
    // Add click event listeners to all buttons and track them
    document.addEventListener('click', (e) => {
        ...
        if (e.target.tagName === 'BUTTON' || e.target.onclick) {
            const logData2 = {
                ...
                runId: 'run1',
                hypothesisId: 'G',
                location: 'app.js:1285',
                message: 'Button click detected',
                data: {
                    tagName: e.target.tagName,
                    id: e.target.id || 'none',
                    className: e.target.className || 'none',
                    hasOnclick: !!e.target.onclick,
                    onclickType: typeof e.target.onclick,
                    computedStyle: {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const motecTbody = document.getElementById('motec-files-tbody');
    if (motecTbody) {
        loadMotecFiles();
    }
    
    // Load cars for selectors
    loadCarsForSelector();
    loadCarsForQueueFilter();
    
    // Setup upload form (check both possible locations)
    ...
    // Also allow direct file input change to show filename and enable upload
    const fileInput = document.getElementById('motec-file-input');
    const fileNameDisplay = document.getElementById('motec-file-name');
    if (fileInput) {
        fileInput.addEventListener('change', async (e) => {
            if (e.target.files && e.target.files.length > 0) {
                ...
                if (fileNameDisplay) {
                    ...
                    fileNameDisplay.style.color = 'var(--word-text)';
                }
                // Auto-upload when file is selected (optional - user can also click Upload button)
                // await uploadMotecFile();
            } else {
                if (fileNameDisplay) {
                    fileNameDisplay.textContent = 'No file selected';
                    fileNameDisplay.style.color = 'var(--word-text-secondary)';
                }
            }
        });
    }
