...
# Default subteams (configurable)
DEFAULT_SUBTEAMS = ["Aero", "Engine", "Suspension", "Electronics", "Chassis", "Powertrain", "Data Acquisition"]
...
async def index(request: Request):
    ...
    role = await get_current_user_role(request) or "user"
    ...
    queue_items = []
    if role == "admin":
        queue_items = await get_queue(status="pending")
    ...
    return templates.TemplateResponse(
        "index.html",
        {
            ...
            "subteams": subteams,
            "default_subteams": DEFAULT_SUBTEAMS,
...
async def update_parameter_in_ldx_files(
    ...
    try:
        ...
        for file_id in file_ids_to_update:
            ...
            if file_path and file_path.exists() and file_path.suffix.lower() == '.ldx':
                ...
                if file_path:
                    ...
                    elif file_path.suffix.lower() != '.ldx':
...
@app.get("/api/queue")
async def api_get_queue(request: Request, status: Optional[str] = Query(None)):
    """Get queue items"""
    require_auth(request)
    queue_items = await get_queue(status=status)
...
async def api_process_queue(request: Request, form_id: str):
    ...
    username = await require_role(request, "admin")
...
async def api_upload_motec_file(
    ...
    subteam: str = Form("MoTeC"),  # Subteam for auto-created parameters
...
async def api_import_ldx_to_parameters(
    request: Request,
    file_id: str,
    subteam: str = Query("MoTeC", description="Subteam name for imported parameters"),
    ...
    if file_path.suffix.lower() != '.ldx':
        raise HTTPException(status_code=400, detail="File must be .ldx format")
                    ...
                    if subdir_path.exists():
                        for file in subdir_path.glob("*"):
                            if file.is_file() and not file.name.endswith('.bak'):
                                file.unlink()
                                deleted_count += 1
                result["uploaded_files_deleted"] = deleted_count
