...
async def create_user(username: str, password: str, role: str = "user", subteam: Optional[str] = None) -> bool:
    """Create a new user"""
    import hashlib
    from datetime import datetime
    ...
    try:
        ...
        add_registered_user(username, role, created_at, password, subteam)
...
async def get_queue(status: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if status:
            cursor = await db.execute(
                "SELECT * FROM parameter_queue WHERE status = ? ORDER BY submitted_at DESC",
                (status,)
            )
        else:
            cursor = await db.execute(
                "SELECT * FROM parameter_queue ORDER BY submitted_at DESC"
            )
        rows = await cursor.fetchall()
        return [dict(row) for row in rows]
    finally:
...
async def process_queue_item(form_id: str, processed_by: str) -> bool:
    ...
    try:
        # Get queue item
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("SELECT * FROM parameter_queue WHERE form_id = ? AND status = 'pending'", (form_id,))
        ...
        # Update queue status
        await db.execute("UPDATE parameter_queue SET status = 'processed' WHERE form_id = ?", (form_id,))
        await db.commit()
        return True
    finally:
        await db.close()
...
async def reject_queue_item(form_id: str) -> bool:
    ...
    db = await get_db()
    try:
        cursor = await db.execute("UPDATE parameter_queue SET status = 'rejected' WHERE form_id = ?", (form_id,))
        await db.commit()
        return cursor.rowcount > 0
    finally:
        await db.close()
...
async def update_parameter(
    ...
    try:
        ...
        # Get current parameter to capture prior value
